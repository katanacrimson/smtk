...
if (%smtkloaded%) NEQ (1) if (%moddir%) EQU () (
	echo tool.pngsqueeze.bat requires mod directory via argv if SMTk has not been already loaded. exiting...
	set iserror=1
	goto :END
...
call tee.bat : calling node.pngsqueeze.js to compress PNG asset files...
...
if %iserror% EQU 1 (
	call tee.bat : node.pngsqueeze.js appears to have failed. exiting...
	goto :END
) else (
	call tee.bat : PNG files compressed successfully.
)
