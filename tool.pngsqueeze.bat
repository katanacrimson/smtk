...
set iserror=0
set moddir=!argv!
set templogfile=%dirname%\.temp.pngsqueeze.log
pushd %dirname%

if (%smtkloaded%) NEQ (1) if (%moddir%) EQU () (
	echo [tool] tool.pngsqueeze.bat requires mod directory via argv if SMTk has not been already loaded. exiting...
	set iserror=1
	goto :END
...
call tee.bat : [tool] calling node.pngsqueeze.js to compress PNG asset files...
node.exe %dirname%\node.pngsqueeze.js > "%templogfile%" 2>&1
if errorlevel 1 (
	set iserror=1
)
for /f "tokens=*" %%i in (%templogfile%) do (
	call tee.bat : [pngsqueeze] %%i
)
del "%templogfile%"

if %iserror% EQU 1 (
	call tee.bat : [tool] node.pngsqueeze.js appears to have failed. exiting...
	goto :END
) else (
	call tee.bat : [tool] PNG files compressed successfully.
)
